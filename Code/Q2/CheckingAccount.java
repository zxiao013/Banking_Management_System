  ...
  public void withdraw(double amount) {
    if (this.customer instanceof Adult) {
      if (balance < amount) {
        if ((balance + 500) >= amount) {
          balance = balance - amount - 25.03;
          Transaction withdraw1 = new Transaction((byte) 1, amount, 25.03, "withdraw" + amount + ",overdraft" + (amount - balance));
          add(withdraw1);
        }
      }
      if (balance > amount) {
        balance = balance - amount - 0.03;
        Transaction withdraw2 = new Transaction((byte) 1, amount, 0.03, "withdraw" + amount);
        add(withdraw2);
      }
    }
    if (this.customer instanceof Student) {
      if (balance > amount) {
        ...
        Transaction withdraw3 = new Transaction((byte) 1, amount, 0.02, "withdraw" + amount);
        add(withdraw3);
      }
    }
    if (this.customer instanceof Senior) {
      ...
      if (senior.isVIP()) {
        ...
        if (balance < amount) {
          if ((balance + 500) >= amount) {
            balance = balance - amount - 10.02;
            Transaction withdraw4 = new Transaction((byte) 1, amount, 10.02, "withdraw" + amount + ",overdraft" + (amount - balance));
            add(withdraw4);
          }
        }
        if (balance > amount) {
          balance = balance - amount - 0.01;
          Transaction withdraw5 = new Transaction((byte) 1, amount, 0.01, "withdraw" + amount);
          add(withdraw5);
        }
      }
